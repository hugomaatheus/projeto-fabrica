...
<body>
	...
	<form method="post" action="<c:url value="/usuarios/save"></c:url>">
		<input type="hidden" name="usuario.id" value="${exame.id}"> <label>Nome
			do Exame: </label> <br> <br>
		<%-- 	<input name="usuario.nome" value="${exame.nome}" type="text"> --%>
		<textarea name="usuario.nome" value="${exame.nome}" rows="2" cols="50"></textarea>
		<div></div>
		<br> <label>Descricao do(s) Resultado(s): </label> <br> <br>
		<%-- 	<input name="usuario.resultado" value="${exame	.resultado}" type="text"> --%>
		<textarea name="usuario.resultado" value="${exame.resultado}"
